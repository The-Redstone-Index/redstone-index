...
<button
	class="chip variant-soft-surface flex items-center gap-2 p-2 justify-start w-fit"
	on:click
	type="button"
>
	#{schematicId}
	...
	{#if showLink}
		<a
			href={`/schematics/${schematicId}?blocklist&inputcontrols&elevationslider`}
			target="_blank"
	...
	{#if showDelete}
		<button type="button" on:click={() => dispatch('delete')} class="!mx-0">
