<script lang="ts">
	...
	function openReportTagModal() {
		modalStore.trigger({
			...
			response: async (r) => {
				if (r) {
					const info = {
						reportedUserId: tag.created_by as string,
	...
	function openRecommendModal() {
		modalStore.trigger({
			...
			body: 'Set this tag to recommended category.',
			response: async (r) => {
				if (r) {
					...
					toastStore.trigger({
						message: `<i class="fas fa-check mr-1"></i> Tag Updated!`,
						background: 'variant-filled-success',
						classes: 'pl-8'
					});
				}
	...
	function openUnRecommendModal() {
		modalStore.trigger({
			...
			body: 'Remove this tag from the recommended category.',
			response: async (r) => {
				if (r) {
					const { error } = await supabase
						.from('tags')
