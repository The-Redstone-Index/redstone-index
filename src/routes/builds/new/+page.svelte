...
<div class="container mx-auto p-5">
	...
	<div class="flex gap-10 flex-col mb-10">
				...
				{#each types as t}
					<div>
						...
						<label
							...
							class="inline-flex h-full items-center justify-between w-full p-5  border border-surface-200 dark:border-surface-500 rounded-lg cursor-pointer peer-checked:border-primary-600 peer-checked:text-primary-600 card"
						>
							<div class="block">
		...
		<!-- Specifications -->
		<div class="flex flex-col gap-2">
			<label for="specifications" class="block">Specifications</label>
					...
					<tbody>
						{#each specifications as spec, i}
							<tr class="focus-within:table-row-checked">
								<td contenteditable="true" class="focus:outline-none table-cell-fit">
									{spec.item}
								</td>
		...
		<div class="flex flex-col gap-2">
			...
			<input type="file" class="input" id="schematic-upload" bind:files={schematicFileList} />
		</div>

		<!-- Photos -->
		<div class="flex flex-col gap-2">
			<label for="photos-upload" class="block">Upload Photos (SVG, PNG, JPG, GIF)</label>
			<input id="photos-upload" class="input" type="file" multiple bind:files={photosFileList} />
			{#if photosFileList}
				<ul class="w-fit list">
		...
		<!-- Submit Button -->
		<button class="btn variant-filled-primary md:w-fit">Publish</button>
	</div>
</div>
